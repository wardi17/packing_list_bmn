...
BEGIN
	...
	IF(@status ='Y')
		BEGIN
			...
			FROM  [bmnpt].[dbo].POPAKINGLIST_KURS  WHERE YEAR(EntryDate) =@tahun AND FlagPosting='Y'   ORDER BY EntryDate DESC  ;
		END
	 ELSE
		BEGIN
			INSERT INTO #temptess
			SELECT No_Pls,No_Pli,NoPo,POTransacid,EntryDate,Note,supid,LastUserIDAccess,[bmnpt].[dbo].FunSumDetailPakingList_KURS(No_Pls) as Totaldetail,
			Pib,Forwarder,Total,UserPosting,DatePosting,id_bl_awb,Note2
			FROM  [bmnpt].[dbo].POPAKINGLIST_KURS  WHERE YEAR(EntryDate) =@tahun AND LastUserIDAccess=@userid AND FlagPosting='Y'  ORDER BY EntryDate DESC  ;
		END
